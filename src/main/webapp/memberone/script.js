...
function idCheck(id){
	if(id==""){
		...
		window.open(url,"post","width=300,heigth=150");
	}
	console.log("1");
...
function zipCheck(){
		...
		window.open(url,"post","toolbar=no,width=500,heigth=300,directories=no,status=ys,scrollbars=yes,menubar=no");
...
function inputCheck(){
	console.log(document.regForm.idcheck.value);
	if(document.regForm.id.value==""){
	alert("아이디를 입력해 주세요");
	document.regForm.id.focus();
	return;
	}
	if(document.regForm.idcheck.value=="false"){
		alert("아이디 체크를 해주세요");
		return;
	}
	...
	if(atPos>1&&atPos==atLastPos&&dotPos>3&&spacePos==-1&&commaPos==-1&&atPos+1<dotPos&&dotPos+1<eMailSize);else{
